...
(function (factory) {
	...
	$.fn.jR3DCarousel = function(options){
		...
		(function setup(){
			...
			function _createControls(){
				...
				_previousButton.add(_nextButton).appendTo(_container)
							   .css({position: 'absolute', top:'42%', zIndex:1, display: 'inline-block', padding: '1.2em', boxShadow: '2px 2px 0 #FC7700 inset', cursor:'pointer'})
							   .hide();
			...
			function _createNavigation(){
				...
				_navigation.find('.nav').css({ display: 'inline-block', margin: '5px', cursor: 'pointer', backgroundColor: '#ececec', width: '12px', height: '12px', transition: 'all '+_settings.animationDuration+'ms ease' })
										.first().css({ backgroundColor: '#FC7700' });
		...
		function _slideCarouseld(){
			/* set active nav icon */
			_container.find('.nav').css({  backgroundColor: '#ececec' })
			   .eq(_targetSlideIndex % _noOfSlides).css({ backgroundColor: '#FC7700' });
			_currentSlideIndex = (Math.round(_rotationAngle /_baseAngle) - 1) % _noOfSlides;
			_settings.onSlideShow.call(this, _getNextSlide());
		}
